...
public class ReportListener implements IReporter {
	...
	private static String suiteName = "Master Suite";
	private static String timeZone = "GMT-7";
	private static SimpleDateFormat sdfdate = new SimpleDateFormat("MM-dd-yyyy hh:mm:ss a");
	private static SimpleDateFormat sdftime = new SimpleDateFormat("HH:mm:ss a");
	private static String outFilename = suiteName.replace(" ", "_") + ".html";
	...
	protected void writeHead() {
		...
		writer.print("<head>");
		writer.print("<title>" + suiteName + " Report</title>");
		writeStylesheet();
		writer.print("</head>");
	}
	...
	protected void writeStylesheet() {
		...
		writer.print(".totop {font-size:85%;text-align:center;border-bottom:1px solid #000}");
		writer.print("</style>");
	}
	...
	protected void writeReportTitle(String title) {
		...
		writer.print("<h2>" + title + " <span style=\"\">(" + getDateAsString() + ")</span></h2>");
	}
		...
		if (!classResults.isEmpty()) {
			...
			for (ClassResult classResult : classResults) {
				...
				for (MethodResult methodResult : classResult.getMethodResults()) {
					...
					buffer.append("<td rowspan=\"1\">" + aResult.getName() + "</td>");
					buffer.append("<td rowspan=\"").append(resultsCount).append("\">")
							.append(parseUnixTimeToTimeOfDay(start)).append("</td>");
					buffer.append("<td rowspan=\"").append(resultsCount).append("\">")
							.append(integerFormat.format(duration)).append("</td>");
					buffer.append("</tr>");
					scenarioIndex++;
	...
	private void writeScenario(int scenarioIndex, String label, ITestResult result) {
		...
		writer.print("</table>");
		writer.print("<p class=\"totop\"><a href=\"#\">back to summary</a></p>");
	}
	...
	public static void setSuiteName(String suiteName) {
		...
		ReportListener.suiteName = suiteName;
	}
